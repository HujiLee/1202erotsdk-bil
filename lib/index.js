...
const jsonbeautify = require("json-beautify").default;
...
const ApplicationV2 = require("./ApplicationV2");
 ...
 * @returns {Promise<{ok:Boolean,msg:String,data:{
  ...
  * }}>}
   ...
   * @param {Number|String} history_id 
   * @returns {Promise<{ok:Boolean,msg:String,data:{
   * link:String
   * }}>}
   */
  APIV3_get_history_download_FILES(file_id, history_id) {
    return new Promise(resolve => {
      this.axios.get(`https://drive.kdocs.cn/api/v3/files/${file_id}/histories/${history_id}/download?groupid=${this.mycloud_groupid}`, {
        headers: {
          cookie: this.cookie_as_header
        }
      }).then(axresp => {
        if (axresp.data.result == "ok" && axresp.data.fileinfo && axresp.data.fileinfo.url) {
          return resolve({
            ok: true,
            msg: "ok",
            data: {
    ...
    return new Promise(async resolve => {
      ...
      form.submit({
        ...
        if (!response.headers.etag) {
          ...
          console.log(jsonbeautify(response.headers,null,2,20));
          return 0;
        }
 ...
 * @returns {Promise<{
  ...
  */
   ...
   * }>}
   */
  APIV5_history_set_tag(file_id, fver, tag_name) {

    //不允许的特殊符号\/:*?"&lt;&gt;|
    let safe_tagname = tag_name;
    let disallow_characters = ['\\', '/', ':', '*', '?', '"', '<', '>', '|'];
    let safe_characters = disallow_characters.map(e => Jaco.toWide(e));
    for (let i in disallow_characters) {
      while (safe_tagname.includes(disallow_characters[i])) {
        safe_tagname = safe_tagname.replace(disallow_characters[i], safe_tagname[i])
      }
    }
    if (!safe_tagname.length) {
      safe_tagname += Jaco.toWide('' + Math.random());
    }

    return new Promise(resolve => {
      this.axios.post(`https://drive.kdocs.cn/api/v5/files/${file_id}/histories/tag`, {
        csrfmiddlewaretoken: this.csrf_token,
        file_id: file_id,
        file_ver: fver,
        scene: 0,
        tag_name: safe_tagname
      }, {
        headers: {
          cookie: this.cookie_as_header,
          Origin: `https://www.kdocs.cn`,
...
class FuzaApplication {
  ...
  constructor(kd) {
    ...
    this.v2 = new ApplicationV2(this);
  }
  ...
  async getAvailableMhtFileid(parent_id = 0) {
    ...
    let mhtFiles = o_list.data.files.filter(e => e.ftype == "file" && e.fname.endsWith(".mht") && e.fver <= 30);
 ...
 */
  async smallDummyFileToCoverFile(fileid, parent_id = 0, fname = "") {
    let NO = rn({ integer: true, min: 1, max: 7 });
