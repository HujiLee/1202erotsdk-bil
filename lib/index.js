...
class FuzaApplication {
  ...
  async smallMhtToCoverFileV2(fileid, parent_id = 0, fname = "") {
    ...
    let o_post_ksyun = await this.kd.POST_WPSFILE_KSYUN(cStream, combiedSize,
  ...
  uploadRarFileTask(rar_path, parent_id = 0) {
    ...
    let taskGen = () => new Promise(async task_finish => {
      ...
      let step3_post = await this.kd.POST_WPSFILE_KSYUN(cStream, combined_size, "multipart/related", step1.data.fname, step1.data.parent_id);
      ...
      if (!step4_as_newver.ok) {
        ev.emit(EVENT_NAMES.error, `PUT_as_new_version:${step4_as_newver.msg}`)
        return task_finish();
      }
      console.log("fver", step4_as_newver.data.fver, "\n", "fsha", step4_as_newver.data.fsha);
      await new Promise(r => setTimeout(r, 5000))
      let log_his = await this.kd.APIV3_get_file_histories(step1.data.fileid);
      debugger
      ev.emit(EVENT_NAMES.state_change, "覆盖随机MHT文件");
      let step5_cover = await this.kd.app.smallMhtToCoverFileV2(step1.data.fileid, step1.data.parent_id, step1.data.fname);
      if (!step5_cover.ok) {
      ...
      let my_fver = step4_as_newver.data.fver;
      let my_history = step6_his.data.histories.find(e => {
        if (e.fver == my_fver) {
          // debugger
          return true
        }
        if (step4_as_newver.data.fsha === e.fsha) {
          // debugger
          return true;
        }
        return false;
      ...
      ev.emit(EVENT_NAMES.complete, {
        ...
        fsha: step4_as_newver.data.fsha,
        fname: step4_as_newver.data.fname,
        fsize: combined_size
      });
      return task_finish();
