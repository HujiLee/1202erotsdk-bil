  ...
  uploadUploadifiedDir(dir_path, cloud_parent_id = 0) {
    return new Promise(async resolve => {
      ...
      for (let rarIndex in rarFiles) {
        ...
        for (let tryCount = 1; tryCount <= 4; tryCount++) {
          let isUploadOK = await new Promise(async updone => {
            ...
            ev.on(EVENT_NAMES.error, (err) => {
              // debugger
              console.log(rar.relative_path, err)
              updone(false);
            });
            ev.on(EVENT_NAMES.speed, (args) => {
              // console.log(rar.relative_path, args.speed_text)
              speedTextTmp = `${rar.relative_path}:${args.speed_text}`;
              // rl.write(speedTextTmp+"\n");
              process.stdout.write(speedTextTmp,"utf-8");
              readline.moveCursor(process.stdout,0-speedTextTmp.length,0);
            });
        ...
        if (!o_setTag.ok) {
          let o_setTagRedo = await this.v1p.kd.APIV5_history_set_tag(putAsNewVer.data.fileid, putAsNewVer.data.fver, `V${putAsNewVer.data.fver}-RETRY`);
          if (o_setTagRedo.ok) {

          } else {
            return resolve({
              ok: false,
              msg: `set history fail v1:${o_setTag.msg}\nset history fail v2:${o_setTagRedo.msg}`
            })
          }
